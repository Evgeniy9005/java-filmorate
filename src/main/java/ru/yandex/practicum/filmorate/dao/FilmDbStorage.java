...
public class FilmDbStorage implements FilmStorage {
   ...
   /* private int[]  argTypes  = new int[] {
    ...
    public int addFilm(Film film) {
        ...
        log.info("Добавлено количество строк = {}, при добавлении фильма \"{}\" фильма",row,film);
    ...
    public List<Film> getFilms() {
        ...
        List<Integer> listFilms = jdbcTemplate.queryForList("SELECT FILM_ID FROM PUBLIC.FILMS ORDER BY FILM_ID ASC;", Integer.class);
    ...
    private void upFilmGenres(Set<Genre> genres, int filmId) throws DataAccessException {
        ...
        if (genres == null || genres.isEmpty()) {
            ...
            genres.stream().filter(g -> isGenre(g.getId())) //проверить что такие жанры есть
                    // .filter(g -> isFilmHaveGenre(filmId, g.getId())) //проверить какие жанры добавлены
    ...
    private Set<Genre> getFilmGenre(int filmId) {
        String sqlGenres = "SELECT GENRE_ID FROM PUBLIC.GENRES WHERE FILM_ID = ?;";
    ...
    public Film updateFilm(Film film) {
        ...
        //Список оценок остается прежним
